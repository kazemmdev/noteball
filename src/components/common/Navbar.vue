...
<style scoped>
.navbar {
  @apply fixed flex items-center inset-x-0 top-0 w-full h-[54px] px-4 md:px-0 bg-white shadow-sm;
}
.nav-inner {
  @apply w-full flex items-center justify-between gap-4 max-w-2xl mx-auto;
}
.nav-link {
  @apply text-sm text-center active:text-green-500 hover:text-green-700 transition-all;
}
.nav-link-active {
  @apply bg-green-200/40 text-green-600 px-2 rounded-full;
}
.nav-user {
  @apply flex items-center gap-2 bg-indigo-100/70 py-1 px-2 rounded-full text-xs text-gray-800;
}
</style>
